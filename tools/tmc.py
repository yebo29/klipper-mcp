...
def register_tmc_tools(mcp):
    ...
    async def get_tmc_status(stepper: Optional[str] = None) -> str:
        ...
        if stepper:
            ...
            if not tmc_objects:
                return json.dumps(
                    {"error": f"No TMC driver found for stepper: {stepper}"}, indent=2
                )
        ...
        return json.dumps(
            {"tmc_drivers": tmc_status, "count": len(tmc_status)}, indent=2
        )
    ...
    async def set_tmc_current(
        ...
        if not run_current and not hold_current:
            return json.dumps(
                {"error": "Must specify at least run_current or hold_current"}, indent=2
            )
        ...
        try:
            ...
            if tmc_obj:
                result = await client.get(
                    f'/printer/objects/query?{tmc_obj.replace(" ", "%20")}'
                )
    ...
    async def dump_tmc_registers(stepper: str) -> str:
        ...
        if not tmc_obj:
            return json.dumps(
                {"error": f"No TMC driver found for: {stepper}"}, indent=2
            )
...
""",
        ...
        query_params = "&".join(
            [f'{obj.replace(" ", "%20")}' for obj in autotune_objects]
        )
        result = await client.get(f"/printer/objects/query?{query_params}")
        status = result.get("result", {}).get("status", {})
    ...
    async def list_tmc_steppers() -> str:
        ...
        for obj in all_objects:
            if obj.startswith("tmc"):
                ...
                if len(parts) >= 2:
                    tmc_steppers.append(
                        {"stepper": parts[1], "driver": parts[0].upper()}
                    )
